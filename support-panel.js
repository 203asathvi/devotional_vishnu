...
(function () {
  ...
  var PAGE_KEY  = 'sp_v4_' + location.pathname.replace(/[^a-zA-Z0-9]/g, '_');

  /* ── CSS ── */
  var style = document.createElement('style');
  style.textContent =
    /* FAB — LEFT side, bottom set by JS */
  ...
  /* ── POSITIONING ──────────────────────────────────────────────
     FAB is on the LEFT to avoid right-side conflicts
     (scroll pill + back-to-top are both on the right).

     audioPill is on the left at: left:14px, bottom:20px
     pill height = header(~27px) + audio-row(~45px) = ~72px
     → audioPill top edge = 20 + 72 = 92px from bottom
     → FAB bottom = 92 + 14px gap = 106px on pages with audioPill

     Index page has no pills → FAB at 20px from bottom.
  ─────────────────────────────────────────────────────────────── */
  function placeFab() {
